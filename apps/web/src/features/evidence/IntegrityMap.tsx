...
export function IntegrityMap({ regions, pageGeometry }: Props) {
  const canRender = Boolean(pageGeometry && pageGeometry.width > 0 && pageGeometry.height > 0 && regions.length > 0);
  return (
    <section className="integrity-map" aria-labelledby="integrity-map-title">
      ...
      {canRender ? (
        ...
        <p className="map-unavailable">API chưa cung cấp số trang và hình học trang để đặt chính xác các vùng lên tài liệu, nên giao diện không vẽ bản đồ suy đoán.</p>
      )}
