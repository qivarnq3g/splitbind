...
| Vòng kéo méo của đường cấp phát ảnh là thứ làm hỏng truy vết, không phải bản thân phép biến đổi | `[Experimentally observed]` & `[Limitation]` | Cùng phép so sánh trên; `_canonicalize_page` trong `services/api/splitbind/demo/issuance.py` | Đã kiểm chứng: biến duy nhất khác nhau giữa hai lần thử là tỉ lệ khung hình của tệp đem cấp phát. |
