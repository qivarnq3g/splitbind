...
| Hình 4.4 | Truy nguồn bằng thủy vân khi mã băm không khớp: hệ thống báo hai giá trị mã băm khác nhau nhưng vẫn chỉ đúng bản cấp phát gốc |
...
* **Cấp độ C (Production Capability Layer):** Trên môi trường vận hành thực tế, hệ thống luôn áp dụng cơ chế xác thực toàn vẹn tệp chính xác (Exact-file integrity qua SHA-256) kèm chữ ký Ed25519 trên Manifest. Từ ngày 12/09/2026, nhóm bật thêm phân hệ thủy vân trên chính hệ thống đang chạy: mỗi bản cấp phát nhúng thủy vân V2 và mỗi lần xác minh đều chạy bộ giải mã.
  ...
  * Ngày 13 và 14/09, nhóm tìm ra nguyên nhân gốc thứ ba và sửa. Bản phát hành `integrity-v0.2.1` đang chạy nhận cả PDF, PNG và JPEG ở cả hai chiều cấp phát và xác minh. Về truy vết, phép đo qua đúng giao diện người dùng cho thấy hệ thống truy được nguồn từ một ảnh chụp màn hình đã bị thu nhỏ còn dưới một nửa và có viền đen, với điều kiện bản cấp phát có tỉ lệ khung hình trùng khung chuẩn của hệ thống. Khi bản cấp phát lệch khung chuẩn, hoặc khi tệp bị nén mất dữ liệu, nhóm chưa quan sát được lần truy vết thành công nào. Chi tiết, bảng số liệu và ranh giới ở Mục 4.2.4.
...
Nhưng phép thử quyết định lại nằm ở hai hàng cuối, và chúng đảo ngược kết luận mà nhóm suýt đưa ra.

| Cách ảnh được cấp phát | Phép biến đổi | Kết quả |
|---|---|---|
| Ảnh 1400 x 1980, lệch khung chuẩn | Chụp màn hình 1920 x 1080, thu nhỏ còn 0.545, có viền đen | Không truy được |
| Ảnh 1152 x 2304, đúng khung chuẩn | Chụp màn hình 1920 x 1080, thu nhỏ còn 0.469, có viền đen | **Truy được nguồn** |

Hai hàng này là một phép so sánh có kiểm soát: cùng một lớp tấn công, cùng cách dựng ảnh chụp màn hình, tỉ lệ thu nhỏ gần bằng nhau, và hàng thứ hai còn bị thu nhỏ mạnh hơn. Khác biệt duy nhất là kích thước của tệp đem đi cấp phát.

Kết luận rút ra: **hệ thống đang chạy truy vết được ảnh chụp màn hình đã bị thu nhỏ hơn một nửa, miễn là bản cấp phát không phải đi qua vòng kéo méo.** Ảnh có tỉ lệ khung hình trùng khung chuẩn thì bước canonical hoá thành phép đồng nhất, thủy vân được nhúng và đọc trong cùng một hệ toạ độ, và bộ bóc viền làm đúng việc của nó.

*Đã hiện thực trong mã nguồn* Nguyên nhân chính không nằm ở vật mang mà ở chỗ hệ thống đang chạy và con số 9/12 thuộc về hai thế hệ thuật toán khác nhau. Đường cấp phát và xác minh của dịch vụ gọi `load_v2_profiles`, tức đọc hợp đồng tham số `contracts/algorithm/fingerprint-candidates.v2.json` và ghim ứng viên đầu tiên (`_select_frozen_candidate` trong `services/api/splitbind/demo/issuance.py`). Con số 9/12 trước JPEG-70 lại là của thế hệ V3, đọc từ `fingerprint-candidates.v3.json`. Đặt hai hợp đồng cạnh nhau thì V3 có ba thứ mà V2 hoàn toàn không có: trải phổ với 64 chip cho mỗi bit (`spread_chips_per_bit`), biên độ trải phổ riêng (`spread_delta`), và tám giả thuyết hình học thay vì tối đa ba. Trải phổ chính là thay đổi đã đưa JPEG-70 từ 0/12 lên 9/12, như Câu hỏi 9 đã trình bày.
...
*Đã hiện thực trong mã nguồn* Có một yếu tố thứ hai, và nó giải thích vì sao riêng đường ảnh lại mong manh đến vậy. Đọc mã, `_canonicalize_page` kéo mọi ảnh về đúng một khung cố định 1152 x 2304 điểm ảnh mà không giữ tỉ lệ khung hình. Khung này vốn được chọn cho trang tài liệu dọc, nên một ảnh ngang bị biến dạng rất nặng:
...
Đường đi đầy đủ của một bản cấp phát ảnh vì thế là: kéo về khung chuẩn, nhúng thủy vân trong khung đã méo, rồi kéo ngược về kích thước gốc. Lúc xác minh, ảnh nộp lên lại bị kéo về khung chuẩn một lần nữa trước khi giải mã. Một bản sao giống hệt từng điểm ảnh đi qua đúng chuỗi phép biến đổi ấy nên khớp lại được. Nhưng nếu người nhận thu nhỏ hay nén thêm một lần, phép lấy mẫu lại thứ ba chồng lên một tín hiệu vốn đã qua hai lần biến dạng, và đó là lúc dây chuyền gãy. Phép so sánh có kiểm soát ở trên đo đúng hệ quả này: bỏ vòng kéo méo đi bằng cách cấp phát một ảnh vốn đã đúng khung chuẩn, thì cùng lớp tấn công ấy lại truy vết được.

*Giới hạn đã nhận diện* Có một yếu tố thứ ba cộng thêm vào. Benchmark gọi thẳng bộ giải mã, truyền sẵn số trang và làm việc trên khung ảnh chuẩn tắc, còn dịch vụ nhận một tấm ảnh trần thì không có hai thứ đó và phải tự đoán hình học. Chính Mục 4.2.4.4 đã nêu trước điều này khi nói bảng độ bền mô tả thư viện chứ chưa mô tả sản phẩm; tám phép thử ở đây là phần kiểm chứng thực nghiệm cho nhận định đó.
...
*Giới hạn đã nhận diện* Phát biểu đúng về năng lực truy vết của bản đang chạy vì thế gồm hai vế. Vế khẳng định: hệ thống truy được nguồn từ một ảnh chụp màn hình đã bị thu nhỏ còn dưới một nửa và có viền đen, khi bản cấp phát có tỉ lệ khung hình trùng khung chuẩn. Vế phủ định: khi bản cấp phát lệch khung chuẩn, hoặc khi tệp bị nén mất dữ liệu, nhóm chưa quan sát được lần truy vết thành công nào. Mọi câu trong báo cáo nói rằng bản đang chạy truy vết được tệp đã bị biến đổi đều phải hiểu kèm cả hai vế.
...
| Vòng kéo méo của đường cấp phát ảnh là thứ làm hỏng truy vết, không phải bản thân phép biến đổi | *Số liệu đo thực nghiệm* và *Giới hạn đã nhận diện* | Cùng phép so sánh trên; `_canonicalize_page` trong `services/api/splitbind/demo/issuance.py` | Đã kiểm chứng: biến duy nhất khác nhau giữa hai lần thử là tỉ lệ khung hình của tệp đem cấp phát. |
