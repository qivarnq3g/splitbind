"""Replicated LL-DCT-QIM payload codec for fingerprint V3 tiles."""
...
_PERMUTATION_SEED_DOMAIN = b"splitbind-v3-payload-permutation\x00"
_PERMUTATION_STREAM_DOMAIN = b"splitbind-v3-payload-stream\x00"
...
def embed_codeword_v3(
    ...
    """Embed one interleaved V1 ECC codeword into a V3 tile's LL band."""
    ...
    if replicated_bits.size != len(positions):
        raise ValueError("V3 profile replication does not match payload capacity")
...
def extract_codeword_v3(
    tile_luminance: NDArray[np.float64],
    key: bytes,
    page_index: int,
    profile: FingerprintV3Profile,
) -> CodewordEvidence:
    """Extract an interleaved codeword and its byte erasures from one V3 tile."""
...
def _capacity(profile: FingerprintV3Profile) -> int:
...
def _selected_positions(
    ...
    if required > capacity:
        raise ValueError(f"V3 tile capacity is {capacity} bits, but embedding needs {required}")
    ...
    for _ in range(bit_count):
        ...
        for index, position in enumerate(values):
            ...
            if len(replica_indices) == profile.bit_replication:
                break
        if len(replica_indices) != profile.bit_replication:
            raise ValueError("V3 tile cannot place replicas in distinct DCT blocks")
...
def _position_components(
    position: int, profile: FingerprintV3Profile
